...
exports.createVendor = asyncHandler(async (req, res) => {
    const vendor = await vendorService.createVendor(req.body, req.user._id, req.requestInfo);

    successResponse(res, 201, "Vendor created successfully.", vendor);
});

exports.updateVendor = asyncHandler(async (req, res) => {
    const vendor = await vendorService.updateVendor(req.params.id, req.body, req.user._id, req.requestInfo);

    successResponse(res, 200, "Vendor updated successfully.", vendor);
});

exports.changeVendorStatus = asyncHandler(async (req, res) => {
    const vendor = await vendorService.changeVendorStatus(req.params.id, req.user._id, req.requestInfo);

    successResponse(res, 200, "Vendor status updated successfully.", vendor);
});

exports.deleteVendor = asyncHandler(async (req, res) => {
    await vendorService.deleteVendor(req.params.id, req.user._id, req.requestInfo);
